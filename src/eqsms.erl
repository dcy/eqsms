...
send_sms(NationCode, Mobile, Msg) ->
    ...
    send_sms(Sdkappid, NationCode, Mobile, Msg).

send_sms(Sdkappid, NationCode, Mobile, Msg) ->
    Host = <<"https://yun.tim.qq.com/">>,
    Path = <<"v5/tlssmssvr/sendsms">>,
    Random = eutil:to_binary(random:uniform(10000)),
    URL = hackney_url:make_url(Host, Path, [{<<"sdkappid">>, Sdkappid}, {<<"random">>, Random}]),
    ?TRACE_VAR(URL),
    Time = erlang:system_time(seconds),
    Sig = gen_sms_sig(Sdkappid, Random, Time, Mobile),
    ?TRACE_VAR(Sig),
...
send_isms(Tel, Msg) ->
    {ok, Sdkappid} = application:get_env(eqsms, sdkappid),
    send_sms(Sdkappid, Tel, Msg).

send_isms(Sdkappid, Tel, Msg) ->
    Host = <<"https://yun.tim.qq.com/">>,
    ...
    ?TRACE_VAR(URL),
    Time = erlang:system_time(seconds),
    Sig = gen_isms_sig(Sdkappid, Random, Time, Tel),
    ?TRACE_VAR(Sig),
...
send_multi_sms(Tels, Msg) ->
    {ok, Sdkappid} = application:get_env(eqsms, sdkappid),
    send_multi_sms(Sdkappid, Tels, Msg).
...
send_multi_sms(Sdkappid, Tels, Msg) ->
    ...
    ?TRACE_VAR(URL),
    Time = erlang:system_time(seconds),
    Sig = gen_isms_sig(Sdkappid, Random, Time, Tels),
    ?TRACE_VAR(Sig),
...
gen_multi_sig(Appkey, Random, Time, Tels) ->
    Mobiles = [maps:get(mobile, Tel) || Tel <- Tels],
